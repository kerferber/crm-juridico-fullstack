...
const getEnvBoolean = (value: string | undefined, fallback = false) => {
...
export const initRealtime = ({ token, tenantSlug }: InitRealtimeOptions) => {
  ...
  const wsPath =
    typeof wsPathRaw === 'string' && wsPathRaw.trim().length > 0
      ? wsPathRaw.trim()
      : undefined;
...
type TenantChannelCallbacks = {
  onTaskCreated?: (payload: any) => void;
  onTaskUpdated?: (payload: any) => void;
  onTaskDeleted?: (payload: any) => void;
  onLawsuitCreated?: (payload: any) => void;
  onLawsuitUpdated?: (payload: any) => void;
  onLawsuitDeleted?: (payload: any) => void;
  onContactCreated?: (payload: any) => void;
  onContactUpdated?: (payload: any) => void;
  onContactDeleted?: (payload: any) => void;
  onTransactionCreated?: (payload: any) => void;
  onTransactionUpdated?: (payload: any) => void;
  onTransactionDeleted?: (payload: any) => void;
  onCalendarEventCreated?: (payload: any) => void;
  onCalendarEventUpdated?: (payload: any) => void;
  onCalendarEventDeleted?: (payload: any) => void;
  onSocialPostCreated?: (payload: any) => void;
  onSocialPostDeleted?: (payload: any) => void;
  onSocialCommentCreated?: (payload: any) => void;
  onSocialCommentDeleted?: (payload: any) => void;
  onSocialLikeUpdated?: (payload: any) => void;
  onNotificationCreated?: (payload: any) => void;
};

export const subscribeToTenantChannels = (
  tenantId: number,
  userId: number,
  callbacks: TenantChannelCallbacks
): (() => void) => {
  if (!echo) {
    return () => {};
  }

  const subscriptions: Array<{ channel: ReturnType<Echo<'pusher'>['private']>; events: Array<{ name: string; callback: (...args: any[]) => void }> }> = [];

  const subscribe = (channelName: string, events: Array<{ name: string; callback?: (payload: any) => void }>) => {
    const channel = echo!.private(channelName);
    const registered: Array<{ name: string; callback: (...args: any[]) => void }> = [];
